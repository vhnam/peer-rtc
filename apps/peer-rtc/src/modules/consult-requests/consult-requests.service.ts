...
export async function listConsultRequests(params: ConsultRequestListParams): Promise<ConsultRequestListResult> {
  ...
  url.searchParams.set('consumerId', params.consumerId);
...
export async function createConsultRequest(payload: CreateConsultRequestPayload): Promise<void> {
  const url = new URL('/api/consult-requests', env.VITE_PUBLIC_AUTH_URL);
  ...
  if (!response.ok) {
    throw new Error(`Failed to create consult request (${response.status})`);
  }
}
